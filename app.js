...
function renderTrend(rows){
  ...
  // Trend scope keeps all months in the selected year/region/area/channel/product/shop,
  // while KPI tables remain locked to the selected month.
  let trendRows = DATA.filter(r=>{
    const checks = [
      ["fYear", r.YEAR],["fRegion", r.AREA_GROUP],["fArea", r.AREA],
      ["fChannel", r.CHANNEL],["fProduct", r.PRODUCT],["fShop", r.SHOP_NAME]
    ];
    return checks.every(([id,val])=>!$(id).value || String(val)===$(id).value);
  });
  if(!$("fYear").value) trendRows = DATA.slice();

  const d=monthly(trendRows);
  const max=Math.max(...d.map(x=>x.a),...d.map(x=>x.t),1);
