...
function refreshDependent(){
  const current={};
  Object.keys(filters).forEach(id=>current[id]=$(id).value);
  let rows=DATA;
  ["fYear","fMonth","fRegion","fArea","fChannel","fProduct","fShop"].forEach(id=>{
    const val=current[id]; if(val) rows=rows.filter(r=>String(filters[id](r))===val);
    if(id==="fYear") setOptions("fMonth",rows.map(r=>r.MONTH),"All Months");
    if(id==="fRegion") setOptions("fArea",rows.map(r=>r.AREA),"All Areas");
    if(id==="fArea") setOptions("fChannel",rows.map(r=>r.CHANNEL),"All Channels");
    if(id==="fChannel") setOptions("fProduct",rows.map(r=>r.PRODUCT),"All Products");
    if(id==="fProduct") setOptions("fShop",rows.map(r=>r.SHOP_NAME),"All Shops");
  });
}
function populate(){
  ...
  $("fYear").value="2026"; $("fMonth").value="8";
}
...
function renderTrend(rows){
  const d=monthly(rows), max=Math.max(...d.map(x=>x.a),...d.map(x=>x.t),1);
  $("trendChart").innerHTML=`<div class="bars">${d.map(x=>`<div class="chartcol"><div class="barwrap"><div class="bar actual" style="height:${x.a/max*100}%"></div><div class="bar target" style="height:${x.t/max*100}%"></div></div><div class="barlabel">${x.k}</div></div>`).join("")}</div><div class="legend">Actual ■ &nbsp; Target ■</div>`;
  const maxq=Math.max(...d.map(x=>x.q),...d.map(x=>x.tq),1);
  $("volumeChart").innerHTML=`<div class="bars">${d.map(x=>`<div class="chartcol"><div class="barwrap"><div class="bar qty" style="height:${x.q/maxq*100}%"></div><div class="bar target" style="height:${x.tq/maxq*100}%"></div></div><div class="barlabel">${x.k}</div></div>`).join("")}</div><div class="legend">Actual Volume ■ &nbsp; Target Volume ■</div>`;
}
...
function render(){
  ...
  updateKPIs(rows); renderTrend(rows); renderProduct(rows); renderArea(rows); renderShops(rows);
}
